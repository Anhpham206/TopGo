...
| :--- | :--- |
| [`api.js`](api.js) | **Service Layer**: Trừu tượng hóa kết nối API. Quản lý việc gửi yêu cầu lên Backend (hoặc kích hoạt fallback offline khi gặp sự cố mạng). |
| [`main.js`](main.js) | **Controller / Orchestrator**: Trái tim điều phối luồng dữ liệu, xử lý tương tác của người dùng trên form tạo lịch trình và chuyển đổi trạng thái ứng dụng. |
| [`ui.js`](ui.js) | **Presenter Layer**: Chuyên biệt render giao diện (gợi ý địa điểm, chi phí, thẻ thời tiết, thẻ món ăn, và timeline lịch trình). |
| [`chatbot.js`](chatbot.js) | **AI Assistant UI & Logic**: Tương tác với API Chatbot và kiểm soát phản hồi nhanh dựa trên bộ từ khóa kịch bản ngoại tuyến khi mất kết nối. |
| [`map.js`](map.js) | **Map Integration**: Tích hợp bản đồ Leaflet, vẽ các tuyến đường di chuyển tối ưu giữa các điểm du lịch. |
| [`data.js`](data.js) | **Business Constants**: Chứa các hằng số nghiệp vụ như ma trận khoảng cách km thật (`CITY_DIST`), đơn giá cơ bản và NLP nhận diện địa danh khởi hành. |
| [`mockFallback.js`](mockFallback.js) | **Fallback Storage**: Lưu trữ dữ liệu dự phòng cục bộ cho Chatbot và danh sách địa điểm offline khi API Backend không phản hồi. |
| [`utils.js`](utils.js) | **Helper Library**: Các hàm xử lý ngày tháng, định dạng tiền tệ (VND), tạo hiệu ứng gõ chữ (typing effect), debouncing, v.v. |
| [`auth.js`](auth.js) | **Authentication**: Xử lý đăng ký, đăng nhập, phân quyền người dùng và lưu trữ JWT token / Firebase Auth state. |
| [`fragmentLoader.js`](fragmentLoader.js) | **Dynamic UI Loader**: Tải động các phân mảnh HTML (Header, Footer, Modals) để giữ code sạch sẽ và tránh trùng lặp. |
| [`newsfeed.js`](newsfeed.js) | **Social Feed Logic**: Quản lý dòng thời gian mạng xã hội du lịch, tải các bài đăng, xử lý lượt thích, bình luận và chia sẻ bài đăng. |
| [`post.js`](post.js) | **Post Detail Controller**: Xử lý logic hiển thị chi tiết bài đăng và danh sách các bình luận liên quan. |
| [`postCard.js`](postCard.js) | **UI Post Component**: Render và xử lý sự kiện cho thẻ bài viết mạng xã hội (tái sử dụng tại Newsfeed và Profile). |
| [`pricing.js`](pricing.js) | **VIP Payment Controller**: Điều khiển giao diện lựa chọn gói dịch vụ VIP, gọi API tạo liên kết thanh toán qua VNPay Sandbox. |
| [`profile.js`](profile.js) | **Profile & Passport Book**: Quản lý Hộ chiếu thành viên, tải avatar canvas, chỉnh sửa thông tin cá nhân và hiển thị danh sách lịch trình đã lưu. |
| [`profilePosts.js`](profilePosts.js) | **Profile Social Feed**: Tải và hiển thị danh sách bài viết cá nhân của người dùng trên trang Profile. |
| [`location.js`](location.js) | **Location Page Controller**: Hiển thị thông tin chi tiết địa điểm và các bài đăng liên kết với địa danh đó. |
| [`shared.js`](shared.js) | **Global Shared Core**: Chứa logic dùng chung như khởi tạo Header/Footer, quản lý Toast thông báo và Event Delegation toàn cục. |
| [`darkmode.js`](darkmode.js) | **Theme Switcher**: Quản lý lưu trữ trạng thái và áp dụng giao diện tối/sáng (Dark Mode) cho toàn website. |
| [`sidebarUtils.js`](sidebarUtils.js) | **Sidebar Handler**: Xử lý hiển thị đóng/mở và trượt của các menu bên (địa điểm nổi bật, gợi ý kết bạn) trên trang feed. |
| [`aboutus.js`](aboutus.js) | **About Us Core**: Tạo các hoạt ảnh và tương tác trên trang giới thiệu đội ngũ. |
| [`home.js`](home.js) | **Landing Page Core**: Quản lý các hiệu ứng động và hiển thị trên trang chủ giới thiệu. |
| [`homeMissionVision.js`](homeMissionVision.js) / [`missionVision.js`](missionVision.js) | **Mission/Vision Logic**: Xử lý hiển thị hoạt họa phần Sứ mệnh & Tầm nhìn tại trang chủ. |
| [`supabaseConfig.js`](supabaseConfig.js) | **Supabase SDK Connection**: File cấu hình kết nối SDK Supabase (phục vụ dữ liệu phụ trợ / dự phòng). |
...
1. **Lazy Loading**: Tệp `mockFallback.js` có dung lượng tương đối lớn. Nó **không** được tải mặc định khi tải trang. Chỉ khi phát hiện lỗi HTTP Request lên Backend, hàm `loadMockFallback()` trong [`api.js`](api.js) mới âm thầm chèn thẻ `<script>` để tải dữ liệu dự phòng. Điều này tối ưu hóa 100% thời gian phản hồi ban đầu của trang (LCP).
...
2. **Mảng `MOCK_CITIES` (trong `mockFallback.js`)**:
   ...
   - *Lý do xóa*: Đã được thay thế hoàn toàn bởi danh sách tĩnh chuẩn có bổ sung tiêu đề mô tả ngắn (subtitle) và màu sắc định danh trực tiếp trong hàm `fetchCities()` của [`api.js`](api.js). `MOCK_CITIES` trở thành mã nguồn chết (Dead Code).
...
Chúng tôi đã sử dụng tập lệnh tự động hóa Node.js để phân tích cú pháp tĩnh tệp [`mockFallback.js`](mockFallback.js):
...
**Trước cải tiến ([`api.js:L24`](api.js#L24)):**
```javascript
...
1. **Hiệu suất & Dung lượng**: File [`mockFallback.js`](mockFallback.js) giảm dung lượng đáng kể (từ **354 KB** xuống còn **142 KB**), tiết kiệm hơn **60%** dung lượng tải tệp nhưng bảo toàn 100% tính năng dự phòng.
